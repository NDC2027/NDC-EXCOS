...
## INEC Wards and Polling Units integration

This version replaces the old sample location object with the INEC Polling Unit directory flow:

State → Local Government Area → Ward / Registration Area → Polling Unit → Delimitation Code.

The portal first requests the current lists from the official INEC polling-unit endpoints used by the INEC Polling Units page. Because some browsers/GitHub Pages deployments can be blocked by cross-origin restrictions on the INEC site, the code includes a fallback to a public structured snapshot of the same INEC directory (37 states/FCT, 774 LGAs, 8,809 wards/registration areas and 176,846 polling units, scraped in 2025). The selected polling unit's delimitation code is filled automatically.

The location module is `inec-locations.js`. It caches lists locally for 24 hours to reduce repeated requests. The "Refresh INEC Lists" button clears that cache and requests fresh data again.

No manual ward/polling-unit list is hard-coded in `common.js` anymore.
